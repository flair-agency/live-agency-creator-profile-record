...
export async function resolveProfileSource(args) {
  ...
  const provider = await resolveProvider({
    providers,
    capability: PROFILE_OBSERVATION_CAPABILITY,
    request,
  ...
  const observations = validateProfileObservations(await readFromProvider(provider, request));
...
export async function main(argv = process.argv.slice(2)) {
  try {
    const result = await resolveProfileSource(parseArgs(argv));

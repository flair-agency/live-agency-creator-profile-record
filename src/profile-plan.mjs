// Business planning extracted unchanged from scripts/profile_sync_core.mjs
// at Skill commit 168418af0afdbf78546cf7c68a52e63c024b0316.
...
export function normalizeProfileObservations(snapshot, nowMs) {
...
export function buildProfileSyncPlanFromHistory({
  manifest,
  observations,
  profileHistory,
  nowMs = Date.now(),
  ...
  assert(Array.isArray(profileHistory), "profile history collection is invalid");
  assert(Number.isSafeInteger(nowMs), "nowMs is invalid");
  const normalized = normalizeProfileObservations(observations, nowMs);
